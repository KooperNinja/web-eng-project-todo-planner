	...
	try {
		const todo = await req.context.prisma.todo.update({
			...
			data: {
				...
				endAt:
					duration && startAtMs
						? new Date(startAtMs + duration * 60000)
						: undefined,
			},
		})
		res.json(todo)
...
export const createTodoSmartSchedule = async (req, res) => {
	...
	try {
		duration = duration ?? 30
		const startAt = await calculateSmartStartTime(
			req.context,
			duration,
			timeFrameStartMs,
			timeFrameEndMs
 ...
 */
const calculateSmartStartTime = async (
	context,
	duration,
	timeFrameStartMs,
	timeFrameEndMs
	...
	for (let i = 0; i < todos.length; i++) {
		...
		if (
			todo.startAt.getTime() > endAtMs &&
			(!lastTodo || lastTodo.endAt.getTime() < startAt.getTime())
		) {
			return startAt
		}
